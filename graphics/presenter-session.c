                                   ...
                                   uint32_t height)
{
    ...
    if (!session || buffer_id == 0 || !aurora_presenter_dimensions_valid(width, height) ||
        session->buffer_count >= AURORA_PRESENTER_MAX_BUFFERS ||
        find_id(session->buffers, session->buffer_count, buffer_id) != session->buffer_count ||
        pixels > AURORA_PRESENTER_MAX_REGISTERED_PIXELS - session->registered_pixels) {
        errno = EINVAL;
        return -1;
    }
                                  ...
                                  uint64_t serial, uint64_t buffer_id)
{
    if (!session || serial == 0 ||
        find_id(session->buffers, session->buffer_count, buffer_id) == session->buffer_count ||
        session->inflight_count >= AURORA_PRESENTER_MAX_INFLIGHT_FRAMES ||
