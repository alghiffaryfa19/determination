...
Inside the guest, with this directory staged at `/root/aurorahyprland`:

```sh
sh /root/build-aurorahyprland-buffer.sh
```
...
`guest/fetch-aurorahyprland.sh <destination>` fetches Hyprland v0.49.0 and the
...
Wayland/xkbcommon development packages. `guest/build-aurorahyprland.sh` builds into
`/opt/aurorahyprland`, isolated from the working desktop. Clang 19 and the pinned
...
5. Only then enable the AuroraHyprland manifest in the companion selector.

The current buffer adapter is **not connected to a running Hyprland renderer**.
AuroraHyprland remains unavailable in the selector. The Android SDK is absent on
