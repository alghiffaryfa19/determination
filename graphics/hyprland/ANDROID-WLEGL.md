...
`guest/patch-hyprland.sh` copies the adapter/XML and applies the patch after
...
Artifact: `build/hyprland-wlegl-cross/Hyprland` (about 15 MB).
SHA256: `7b7441d4856d62e0f19311fb9bf165481ecf6e1d09951bc71cda6e0d17ad459a`.
Logs: `build/hyprland-src/wlegl-cross-build.log` and
`build/hyprland-src/wlegl-arm64-version.log`.
...
```
python3 guest/build-hyprland-wlegl-cross.py
```

The script requires the captured `build/hyprland-sysroot` tree (guest
`usr/include`, `usr/local/include`, `usr/local/lib`, `usr/lib/aarch64-linux-gnu`,
`opt/hyprland/include`, `opt/hyprland/lib`, and the pinned Hyprland generated
