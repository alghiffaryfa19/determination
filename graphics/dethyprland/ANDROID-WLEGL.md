...
registration under `DET_HYBRIS`, and native-buffer texture import.
`guest/patch-dethyprland.sh` copies the adapter/XML and applies the patch after
...
Artifact: `build/dethyprland-wlegl-cross/Hyprland` (about 15 MB).
SHA256: `7b7441d4856d62e0f19311fb9bf165481ecf6e1d09951bc71cda6e0d17ad459a`.
Logs: `build/dethyprland-src/wlegl-cross-build.log` and
`build/dethyprland-src/wlegl-arm64-version.log`.
...
```
python3 guest/build-dethyprland-wlegl-cross.py
```

The script requires the captured `build/dethyprland-sysroot` tree (guest
`usr/include`, `usr/local/include`, `usr/local/lib`, `usr/lib/aarch64-linux-gnu`,
`opt/dethyprland/include`, `opt/dethyprland/lib`, and the pinned Hyprland generated
protocols/build-det), plus the patched host Hyprland checkout. It copies the
