# Aurora Zygisk module --- milestone 6
...
1. **SF-death handler suppression** (IMPLEMENTED --- `jni/main.cpp`):
   ...
   `/data/aurora/run/desktop-mode` exists. Zero-latency in-process
...
3. **Summon UX** --- QS tile / gesture in SystemUI wired to
   `/data/aurora/bin/desktop-on|off`, as a hook, not a replaced APK.
4. **Companion root bridge** --- the module stays loaded only in the Aurora
...
Output: `libs/arm64-v8a/libaurora.so` --- packaged into the Magisk
