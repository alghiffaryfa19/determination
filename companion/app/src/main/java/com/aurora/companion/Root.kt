 ...
 * rights" endlessly. Aurora's on-device tree lives at [AURORA].
 */
object Root {
    const val AURORA = "/data/aurora"
    private const val BIN = "$AURORA/bin"
    private const val LXC = "$AURORA/lxc/bin"
    private const val MODDIR = "/data/adb/modules/aurora"
    ...
    /** One session manifest from $AURORA/etc/sessions, already honesty-filtered. */
    ...
    private const val MARK = "__AURORA_DONE__"
    ...
    /** Whether the on-device Aurora tree is installed. */
    ...
    private fun legacyStatus(): Map<String, String> {
        val script = """
            ...
            [ -f $AURORA/run/desktop-mode ] && echo "mode=desktop" || echo "mode=phone"
            echo "distro=${'$'}($BIN/guest-distro active 2>/dev/null || echo debian)"
            $LXC/lxc-info -P $AURORA -n guest 2>/dev/null | grep -q RUNNING && echo "guest=running" || echo "guest=stopped"
            [ -e $AURORA/run/hostagent.pid ] && echo "agent=up" || echo "agent=down"
            echo "installed=${'$'}([ -x $BIN/desktop-on ] && echo yes || echo no)"
            echo "ip=${'$'}($LXC/lxc-info -P $AURORA -n guest 2>/dev/null | awk '/IP:/{print ${'$'}2; exit}')"
            gauge=${'$'}(sed -n 's/^AURORA_BATTERY_GAUGE=//p' $AURORA/etc/device.conf 2>/dev/null | tail -n 1)
    ...
    fun importToGuest(sourcePath: String, displayName: String): Result {
        val safeName = displayName
            ...
            .take(120)
            .ifBlank { "Shared file" }
        val inbox = "$AURORA/active-guest/home/aurora/Downloads/From Android"
    ...
    fun dockPolicy(): Map<String, String> =
        parseKv(run("cat $AURORA/etc/dock.conf 2>/dev/null", 8).out)
    ...
    fun setDockPolicy(trigger: String, autoExit: Boolean): Result {
        ...
        val write = "mkdir -p $AURORA/etc; printf %s ${shellQuote(body)} > " +
            "$AURORA/etc/dock.conf.new; chmod 0644 $AURORA/etc/dock.conf.new; " +
            "mv -f $AURORA/etc/dock.conf.new $AURORA/etc/dock.conf"
        val stop = "kill \$(cat $AURORA/run/dock-watch.pid 2>/dev/null) 2>/dev/null; " +
            "rm -f $AURORA/run/dock-watch.pid $AURORA/run/dock-watch.fired"
        // Start it right away so the user does not need a reboot.
        val start = "if ! kill -0 \$(cat $AURORA/run/dock-watch.pid 2>/dev/null) " +
            "2>/dev/null; then setsid $AURORA/bin/dock-watch >/dev/null 2>&1 & " +
            "echo \$! > $AURORA/run/dock-watch.pid; fi"
        return run(
    ...
    fun recoverDesktop(): Result =
        run("$LXC/lxc-attach -P $AURORA -n guest -- /usr/bin/pkill -TERM phoc", 15)

    /** Stop the guest container (battery saver : it idles at ~0 but holds RAM + wakeups). */
    fun stopGuest(): Result = run("$LXC/lxc-stop -P $AURORA -n guest -t 10", 30)
    ...
    fun setStopGuestOnExitFlag(on: Boolean): Result =
        if (on) run("mkdir -p $AURORA/etc && touch $AURORA/etc/stop-guest-on-exit", 8)
        else run("rm -f $AURORA/etc/stop-guest-on-exit", 8)

    /** Stop + cold-start the guest container (guest-start is idempotent). */
    fun restartGuest(): Result = run(
        "$LXC/lxc-stop -P $AURORA -n guest -t 10 2>/dev/null; $BIN/guest-start", 40
    )
    ...
    fun dpkgStatus(pkgs: List<String>): Map<String, String> {
        ...
        val r = run(
            "$LXC/lxc-attach -P $AURORA -n guest -- /usr/local/bin/aurora-platform " +
    ...
    fun aptInstall(pkg: String): Result {
        ...
        return run(
            "$LXC/lxc-attach -P $AURORA -n guest -- /bin/sh -c " +
                "'aurora-platform package-refresh >/dev/null; aurora-platform package-install $p' 2>&1 | tail -5",
            600
        )
    }
    ...
    fun aptRemove(pkg: String): Result {
        ...
        return run(
            "$LXC/lxc-attach -P $AURORA -n guest -- /bin/sh -c " +
                "'aurora-platform package-remove $p' 2>&1 | tail -5",
            300
        )
    }
    ...
    fun getCompositor(): String =
        run("cat $AURORA/etc/compositor 2>/dev/null", 8).out.trim().ifBlank { "phosh" }

    fun activeSession(): String = run(
        "if [ -f $AURORA/run/desktop-mode ]; then sed -n 's/^id=//p' $AURORA/run/session.active; fi", 8,
    ).out.trim()
    ...
    fun externalRenderer(): String =
        run(". $BIN/device-config; echo ${'$'}AURORA_EXTERNAL_RENDERER", 8)
            .out.trim()
            .ifBlank { "auto" }
    ...
    fun setExternalRenderer(renderer: String): Result {
        ...
        val config = "$AURORA/etc/device.conf"
        val temporary = "$AURORA/etc/device.conf.renderer-new"
        val guestConfig = "$AURORA/active-guest/etc/aurora-device.conf"
        val guestTemporary = "$AURORA/active-guest/etc/aurora-device.conf.renderer-new"
        return run(
            "mkdir -p $AURORA/etc; " +
                "sed '/^AURORA_EXTERNAL_RENDERER=/d' $config 2>/dev/null > $temporary; " +
                "echo 'AURORA_EXTERNAL_RENDERER=$renderer' >> $temporary; " +
                "mv -f $temporary $config; " +
                "if [ -d $AURORA/active-guest/etc ]; then " +
                "sed '/^AURORA_EXTERNAL_RENDERER=/d' $guestConfig 2>/dev/null > $guestTemporary; " +
                "echo 'AURORA_EXTERNAL_RENDERER=$renderer' >> $guestTemporary; " +
    ...
    fun inputMappings(): Map<String, String> =
        run("cat $AURORA/active-guest/etc/aurora/input-actions.conf 2>/dev/null", 8)
            .out.lineSequence()
            .mapNotNull { line ->
    ...
    fun setInputMapping(code: String, action: String): Result {
        ...
        val path = "$AURORA/active-guest/etc/aurora/input-actions.conf"
        return run(
            "install -d -m 0755 $AURORA/active-guest/etc/aurora; " +
