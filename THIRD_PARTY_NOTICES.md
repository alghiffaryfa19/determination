...
[`guest/fetch-hyprland.sh`](guest/fetch-hyprland.sh). A release must record
...
| Hyprland, Aquamarine, Hypr utilities, Hypr protocols, and hyprwayland-scanner | BSD-3-Clause | Hyprland sources fetched by [`guest/fetch-hyprland.sh`](guest/fetch-hyprland.sh); Aurora patches live in [`graphics/hyprland/patches/`](graphics/hyprland/patches). | Retain BSD-3 notices for each upstream project and the local patch provenance. |
...
| Android Wayland EGL protocol | X11-style permissive licence | [`graphics/hyprland/wayland-android.xml`](graphics/hyprland/wayland-android.xml) is derived from Collabora's protocol. | Keep the copyright and permission notice embedded in that XML and in generated derivatives. |
