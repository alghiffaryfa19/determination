...
[[nodiscard]] bool parse_pid(std::string_view value, int *result) {
    std::uint64_t parsed = 0;
...
[[nodiscard]] bool load_journal(const Options &options, Journal *journal,
    ...
    while (std::getline(input, line)) {
        ...
        if (key == "schema") {
            ...
            if (!parse_pid(value, &candidate.owner_pid)) {
...
void usage(std::ostream &out) {
    out << "usage: aurora-audio-owner status [--root PATH]\n"
           "       aurora-audio-owner claim|restore|recover --profile FILE [--apply]\n"
...
[[nodiscard]] bool parse_options(int argc, char **argv, Options *options) {
    ...
    if (options->probe.empty()) options->probe = options->root + "/bin/aurora-audio-probe";
