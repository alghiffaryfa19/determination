...
Aurora's product audio path is Linux applications -> PipeWire -> ALSA
...
`aurora-audio-probe` is the first safety primitive. It is read-only and reports:
...
`aurora-audio-owner` is the journalled ownership coordinator. It accepts only an
