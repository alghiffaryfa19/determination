...
int main(int argc, char **argv)
{
    ...
    ReceiveResult response = transact(options, options.operation, options.payload,
                                      request_id, &error);
    if (!response.ok) {
        std::cerr << "auroractl: receive: " << response.error << '\n';
    ...
    if (status != Status::Ok && status != Status::Accepted && !options.json) {
        std::cerr << "auroractl: " << status_name(status) << '\n';
    }
    ...
    while (monotonic_milliseconds() < deadline) {
        ...
        if (!response.ok) {
            std::cerr << "auroractl: wait status: " << response.error << '\n';
            return exit_code(response.status);
        }
    ...
    std::cerr << "auroractl: transition wait deadline exceeded\n";
