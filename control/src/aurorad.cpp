...
bool parse_options(int argc, char **argv, Options *options)
{
    ...
    if (options->socket.empty()) options->socket = options->root + "/run/aurorad.sock";
                    ...
                    bool guest_endpoint)
{
    ...
    case Operation::Hello:
        response.payload = "{\"service\":\"aurorad\",\"protocol_major\":1,"
...
int main(int argc, char **argv)
{
    ...
    if (!ensure_directory(options.root + "/run", 0750, &error) ||
        ...
        std::cerr << "aurorad: create state directories: " << error << '\n';
        return 1;
    }
    OperationJournal journal(options.root + "/state/operations.jsonl");

    const std::string lock_path = options.root + "/run/aurorad.lock";
    const int lock = open(lock_path.c_str(), O_RDWR | O_CREAT | O_CLOEXEC, 0640);
    if (lock < 0 || flock(lock, LOCK_EX | LOCK_NB) != 0) {
        std::cerr << "aurorad: another instance owns " << lock_path << '\n';
    ...
    if (!controller.initialise(&error)) {
        std::cerr << "aurorad: initialise state: " << error << '\n';
        close(lock);
        return 1;
    }
    ...
    if (server < 0) {
        std::cerr << "aurorad: create socket: " << error << '\n';
        close(lock);
        return 1;
    }
    const std::string guest_socket = options.root + "/run/control/aurorad.sock";
    const int guest_server = create_server(guest_socket, 1000, &error);
    if (guest_server < 0) {
        std::cerr << "aurorad: guest socket unavailable: " << error << '\n';
    }
    ...
    while (running.load()) {
        ...
        if (ready < 0) {
            ...
            std::cerr << "aurorad: poll: " << std::strerror(errno) << '\n';
            break;
        }
        ...
        if (client < 0) {
            if (errno == EINTR) continue;
            std::cerr << "aurorad: accept: " << std::strerror(errno) << '\n';
            continue;
        }
        ucred peer{};
        ...
        if (!send_packet(client, response, &send_error, kClientDeadlineMs)) {
            std::cerr << "aurorad: response: " << send_error << '\n';
        }
        close(client);
        }
    }
