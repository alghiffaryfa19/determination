#!/usr/bin/env python3
"""Classify a Aurora recon report into capabilities and hard gates."""
...
def main() -> int:
    ...
    capabilities = {
        "AURORA_CAP_STATUS": status,
        "AURORA_CAP_COMPOSER": composer,
        "AURORA_CAP_MAPPER": mapper,
        "AURORA_CAP_ALLOCATOR": allocator,
        "AURORA_CAP_GPU": gpu,
        "AURORA_CAP_BINDER": binder,
        "AURORA_CAP_DRM": drm,
        "AURORA_CAP_KERNEL": kernel,
        "AURORA_CAP_BOOT_LAYOUT": boot_layout,
        "AURORA_CAP_KERNEL_MISSING": ",".join(missing_kernel),
    }
    ...
    with (output / "compatibility.txt").open("w") as f:
        f.write(f"Aurora compatibility: {status}\n\n")
        f.write("Detected\n")
        for key, value in capabilities.items():
            if key != "AURORA_CAP_STATUS" and value:
                f.write(f"- {key.removeprefix('AURORA_CAP_').lower()}: {value}\n")
        if blockers:
            f.write("\nBlockers\n")
