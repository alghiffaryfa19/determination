#!/bin/sh
# Aurora device recon (design spec §9). Run on the host with the phone on
...
PANEL=$(ash 'wm size 2>/dev/null' | tr -d '\r' | sed -n 's/.*Physical size: \([0-9][0-9]*\)x\([0-9][0-9]*\).*/\1 \2/p' | head -n 1)
{
    ...
    echo "AURORA_PROFILE_ID=${DEVICE:-unknown}"
    [ -n "$BACKLIGHT" ] && echo "AURORA_BACKLIGHT_PATH=$BACKLIGHT"
    [ -n "$BLMAX" ] && echo "# AURORA_BACKLIGHT_LEVEL=$((BLMAX / 3))"
    [ -n "$WIFI" ] && echo "AURORA_WIFI_IFACE=$WIFI"
    [ -n "$DRM" ] && echo "AURORA_DRM_CARD=$DRM"
    [ -n "$BATTERY" ] && echo "AURORA_BATTERY_GAUGE=$BATTERY"
    if [ -n "$PANEL" ]; then
        echo "AURORA_PANEL_WIDTH=${PANEL%% *}"
        echo "AURORA_PANEL_HEIGHT=${PANEL#* }"
    fi
} > "$OUT/device.conf"
...
echo "Review $OUT/device.conf before installing it as /data/aurora/etc/device.conf."
