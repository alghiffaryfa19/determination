...
- [x] `boot/aurora-boot.img` — real installed boot_b
      (`artifacts/boot_b-crdroid-12.10.img`) repacked with our kernel;
      kernel-inside verified as our build, cmdline preserved
- [x] Magisk module zip (`magisk-module/aurora-magisk-*.zip`) with static
      aarch64 evgrab + toggle scripts payload
- [x] Host toolkit: `./aurora` helper, magiskboot + cross tools in `toolchain/`
...
2. **Magisk-patch the boot image** (on the phone, no cable needed):
   `./aurora connect && adb push boot/aurora-boot.img /sdcard/Download/`
...
4. **Verify while RAM-booted** (`./aurora status` — wants: root ok, our uname,
...
6. **Install the module**: `./aurora push-module`, reboot, `./aurora status` again.
7. Proceed to guest bring-up: `guest/build-rootfs.sh` (needs mmdebstrap on
   the host) + a static arm64 lxc for `/data/aurora/lxc/bin`.
