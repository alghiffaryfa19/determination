#!/system/bin/sh
# One-shot on-device migration: DecemberOS -> Aurora.
...
#
#   aurora migrate            (pushes + runs this over adb), or
#   adb push migrate-to-aurora.sh /sdcard/Download/ &&
#     adb shell "su -c 'sh /sdcard/Download/migrate-to-aurora.sh'"
#
# After it succeeds: install the new (id=aurora) module zip and reboot.
...
# --- Rename guest-side dos-* artifacts to aurora-* -------------------------------
# The new host toggles look for aurora-input-udevdb / aurora-pidfd-shim.so by path.
...
say "NEXT: install the new aurora-magisk-*.zip (Magisk app or 'aurora push-module'),"
