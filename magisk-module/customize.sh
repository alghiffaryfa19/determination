...
for f in aurora-guest-agent aurora-audio-probe aurora-audio-session aurora-pipewire-smoke aurora-input-actions aurora-media-action aurora-connectivity aurora-connectivity-menu aurora-platform aurora-phosh-session aurora-compat-check aurora-firefox-content-defaults aurora-session-launch aurora-plasma-session aurora-plasma-client aurora-hyprland aurora-hyprland-opal aurora-hyprland-omarchy aurora-omarchy aurora-opal aurora-opal-bridge opal; do
...
if [ -d "$MODPATH/guest-assets/omarchy" ]; then
    mkdir -p "$STAGE/guest-assets/omarchy" "$GUEST_ROOT/usr/local/share/aurora-omarchy"
