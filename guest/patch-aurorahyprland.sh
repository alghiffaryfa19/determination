#!/bin/sh
set -eu
SRC=${1:-/root/build/aurorahyprland}
PATCHES=${2:-/root/aurorahyprland/patches}
...
mkdir -p "$SRC/Hyprland/src/aurorahyprland"
cp "$PATCHES/../AndroidWlegl.cpp" "$PATCHES/../wayland-android.xml" \
    "$SRC/Hyprland/src/aurorahyprland/"
mkdir -p "$SRC/aquamarine/src/aurorahyprland"
cp "$PATCHES/../HybrisBuffer.cpp" "$PATCHES/../HybrisBuffer.hpp" \
    "$PATCHES/../AquamarineBuffer.hpp" "$PATCHES/../GrallocAllocator.hpp" \
    "$PATCHES/../HwcPresenter.hpp" "$SRC/aquamarine/src/aurorahyprland/"
