...
Requires build/hyprland-sysroot with guest headers/libraries/build-aurora and
build/hyprland-src/Hyprland with the android_wlegl patch applied. Does not
...
SYSROOT = ROOT / "build/hyprland-sysroot"
SOURCE = ROOT / "build/hyprland-src/Hyprland"
CACHE = SYSROOT / "root/build/hyprland/Hyprland/build-aurora"
OUTPUT = ROOT / "build/hyprland-wlegl-cross"
...
def target_path(path):
    if path.startswith("/root/build/hyprland/Hyprland/"):
        return container_path(SOURCE) + path.removeprefix("/root/build/hyprland/Hyprland")
...
def main():
    ...
    adapter = SOURCE / "src/hyprland"
    adapter.mkdir(parents=True, exist_ok=True)
    shutil.copy2(ROOT / "graphics/hyprland/AndroidWlegl.cpp", adapter / "AndroidWlegl.cpp")
    ...
    while i < len(tokens):
        ...
        flags.append(token)
        i += 1
    protocols = SYSROOT / "root/build/hyprland/Hyprland/protocols"
    flags += ["-I" + container_path(protocols), "-I" + container_path(OUTPUT),
              "-I" + container_path(SYSROOT / "usr/include/android"),
              "-I" + container_path(SYSROOT / "opt/hyprland/include/hyprland/src/render")]
    xml = ROOT / "graphics/hyprland/wayland-android.xml"
    ...
    for source in ("src/render/OpenGL.cpp", "src/render/Renderbuffer.cpp", "src/render/Texture.cpp",
                   "src/managers/ProtocolManager.cpp", "src/hyprland/AndroidWlegl.cpp"):
    ...
    run(["aarch64-linux-gnu-g++", "--sysroot=" + container_path(SYSROOT),
         ...
         "-Wl,-rpath-link," + container_path(SYSROOT / "opt/hyprland/lib"),
         "-Wl,-rpath-link," + container_path(SYSROOT / "usr/lib/aarch64-linux-gnu"),
         objects["CMakeFiles/Hyprland.dir/src/hyprland/AndroidWlegl.cpp.o"], protocol_obj,
