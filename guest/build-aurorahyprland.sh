...
case "${2:-build}" in
    deps)
        if command -v aurora-platform >/dev/null 2>&1 && [ "$(aurora-platform id)" != debian ]; then
            ...
            aurora-platform deps aurorahyprland
            exit
        fi
        ...
        ;;
    build) ;;
    *) echo 'usage: build-aurorahyprland.sh <sources> [deps|build]' >&2; exit 2 ;;
esac
[ -d "$SRC/Hyprland/.git" ] || { echo 'run fetch-aurorahyprland.sh first' >&2; exit 2; }
