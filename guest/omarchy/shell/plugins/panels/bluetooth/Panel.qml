...
// Opal builds without the optional PipeWire QML service; audio routing below
// uses the omarchy-audio-output-set-default command instead.
import qs.Ui
import qs.Commons
...
Panel {
  ...
  readonly property var pipewireNodes: []
  ...
  function setDefaultAudioSink(sink) {
    if (!sink) return
    // No PipeWire QML service in this runtime; the command performs the switch.
