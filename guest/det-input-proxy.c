...
#define DET_MAX_TOUCH_SLOTS 32

struct det_touch_slot {
    int tracking_id;
    int x;
    int y;
    int minimum_x;
...
struct det_source_state {
    ...
    struct det_touch_slot slots[DET_MAX_TOUCH_SLOTS];
};
...
static int send_default_keymap(void)
{
    static const char keymap[] =
        ...
        "};\n";
    int fd = memfd_create("determination-keymap", MFD_CLOEXEC);
    if (fd < 0)
        return -1;
    size_t offset = 0;
...
static int initialize_input_protocols(void)
{
    if (fake_input) {
        org_kde_kwin_fake_input_authenticate(
            fake_input, "Determination", "User enabled peripheral capture");
        fprintf(stderr, "det-input-proxy: using KDE fake-input\n");
        return 0;
    }
    ...
    fprintf(stderr,
            "det-input-proxy: using wlroots virtual keyboard/pointer\n");
    return 0;
}
...
static int connect_touch_socket(const char *path)
{
    ...
    fprintf(stderr, "det-input-proxy: Android display touch connected %s\n",
            path);
    return fd;
}
...
{
    return (source_id - 1u) * DET_MAX_TOUCH_SLOTS + slot;
}

static int flush_direct_touch(struct det_source_state *source,
                              uint32_t source_id, int width, int height)
{
    if (!fake_input || !(source->flags & DET_INPUT_SOURCE_DIRECT))
        return 0;

    int emitted = 0;
    if (source->flags & DET_INPUT_SOURCE_MULTITOUCH) {
        for (uint32_t index = 0; index < DET_MAX_TOUCH_SLOTS; ++index) {
            struct det_touch_slot *slot = &source->slots[index];
...
static void cancel_direct_touch(struct det_source_state *source)
{
    if (fake_input && (source->flags & DET_INPUT_SOURCE_DIRECT))
...
static void flush_source(struct det_source_state *source,
                         uint32_t source_id, int width, int height)
{
...
static int dispatch_packet(const struct det_input_forward_packet *packet,
                           struct det_source_state *sources,
                           int width, int height)
{
    if (packet->magic != DET_INPUT_FORWARD_MAGIC ||
        packet->version != DET_INPUT_FORWARD_VERSION ||
        packet->size != sizeof(*packet) ||
        packet->source_id == 0 ||
        packet->source_id > DET_INPUT_MAX_SOURCES ||
        packet->type > EV_MAX) {
        errno = EPROTO;
        return -1;
    }
    struct det_source_state *source = &sources[packet->source_id - 1];
    ...
    case EV_KEY:
        ...
        if (packet->code == BTN_TOUCH && fake_input &&
            (source->flags & DET_INPUT_SOURCE_DIRECT)) {
            if (!(source->flags & DET_INPUT_SOURCE_MULTITOUCH)) {
    ...
    case EV_ABS:
        if (fake_input && (source->flags & DET_INPUT_SOURCE_DIRECT) &&
            (source->flags & DET_INPUT_SOURCE_MULTITOUCH)) {
            struct det_touch_slot *slot = &source->slots[source->current_slot];
            if (packet->code == ABS_MT_SLOT) {
                if (packet->value >= 0 &&
                    packet->value < DET_MAX_TOUCH_SLOTS)
...
int main(int argc, char **argv)
{
    ...
    if (!display) {
        fprintf(stderr, "det-input-proxy: KWin Wayland socket unavailable\n");
        return 1;
    }
    ...
    if (wl_display_roundtrip(display) < 0 ||
        ...
        fprintf(stderr,
                "det-input-proxy: compositor has no supported input protocol\n");
    ...
    if (input_pipe < 0) {
        fprintf(stderr, "det-input-proxy: create FIFO %s: %s\n",
    ...
    fprintf(stderr, "det-input-proxy: ready fifo=%s geometry=%dx%d\n",
            socket_path, width, height);

    struct det_source_state sources[DET_INPUT_MAX_SOURCES] = {0};
    for (uint32_t source = 0; source < DET_INPUT_MAX_SOURCES; ++source) {
        for (uint32_t slot = 0; slot < DET_MAX_TOUCH_SLOTS; ++slot)
    ...
    while (running) {
        ...
        for (nfds_t index = 0; index < item_count; ++index) {
            if (index == 1 &&
                ...
                reconnect_ticks = 0;
                fprintf(stderr,
                        "det-input-proxy: Android display touch disconnected\n");
                continue;
            }
            ...
            if (received != (ssize_t)sizeof(packet) ||
                ...
                if (index == 1) {
                    fprintf(stderr,
                            "det-input-proxy: rejected Android touch packet "
                    ...
                    reconnect_ticks = 0;
                    cancel_direct_touch(
                        &sources[DET_INPUT_ANDROID_TOUCH_SOURCE_ID - 1u]);
                    continue;
                }
                result = 1;
                break;
