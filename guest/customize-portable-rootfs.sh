...
install -m 0755 "$HERE/aurora-apps" "$ROOT/usr/local/bin/aurora-apps"
if [ "$PROFILE" = arch ] && [ ! -e "$ROOT/etc/aurora-apps-profile" ]; then
    printf '%s\n' base > "$ROOT/etc/aurora-apps-profile"
fi
...
if [ -e /etc/aurora-ready ]; then
    [ "$(aurora-platform id)" != arch ] || aurora-apps apply
    echo "Aurora guest already provisioned"
    exit 0
fi
