...
refresh_linker_cache() {
    ...
    [ "${AURORA_PREPARE_ONLY:-0}" != 1 ] || return 0
...
if [ "${AURORA_PREPARE_ONLY:-0}" = 1 ]; then
    :
elif command -v aurora-platform >/dev/null 2>&1; then
...
cd "$B" && rm -rf libdroid
git init libdroid
git -C libdroid remote add origin "$LIBDROID_REPO"
git -C libdroid fetch --depth 1 origin "$LIBDROID_COMMIT"
git -C libdroid checkout --detach FETCH_HEAD
cd libdroid
...
cd "$B" && rm -rf wlroots
git init wlroots
git -C wlroots remote add origin "$WLROOTS_REPO"
...
cd "$B" && rm -rf phoc
git init phoc
git -C phoc remote add origin "$PHOC_REPO"
git -C phoc fetch --depth 1 origin "$PHOC_COMMIT"
git -C phoc checkout --detach FETCH_HEAD
cd phoc
...
[ "${AURORA_PREPARE_ONLY:-0}" != 1 ] || exit 0
echo "== sanity =="
