...
Requires build/aurorahyprland-sysroot with guest headers/libraries/build-aurora and
build/aurorahyprland-src/Hyprland with the android_wlegl patch applied. Does not
...
SYSROOT = ROOT / "build/aurorahyprland-sysroot"
SOURCE = ROOT / "build/aurorahyprland-src/Hyprland"
CACHE = SYSROOT / "root/build/aurorahyprland/Hyprland/build-aurora"
OUTPUT = ROOT / "build/aurorahyprland-wlegl-cross"
IMAGE = "aurora/quickshell-cross:trixie"
...
def target_path(path):
    if path.startswith("/root/build/aurorahyprland/Hyprland/"):
        return container_path(SOURCE) + path.removeprefix("/root/build/aurorahyprland/Hyprland")
...
def main():
    ...
    adapter = SOURCE / "src/aurorahyprland"
    adapter.mkdir(parents=True, exist_ok=True)
    shutil.copy2(ROOT / "graphics/aurorahyprland/AndroidWlegl.cpp", adapter / "AndroidWlegl.cpp")
    ...
    while i < len(tokens):
        ...
        flags.append(token)
        i += 1
    protocols = SYSROOT / "root/build/aurorahyprland/Hyprland/protocols"
    flags += ["-I" + container_path(protocols), "-I" + container_path(OUTPUT),
              "-I" + container_path(SYSROOT / "usr/include/android"),
              "-I" + container_path(SYSROOT / "opt/aurorahyprland/include/hyprland/src/render")]
    xml = ROOT / "graphics/aurorahyprland/wayland-android.xml"
    ...
    for source in ("src/render/OpenGL.cpp", "src/render/Renderbuffer.cpp", "src/render/Texture.cpp",
                   "src/managers/ProtocolManager.cpp", "src/aurorahyprland/AndroidWlegl.cpp"):
    ...
    run(["aarch64-linux-gnu-g++", "--sysroot=" + container_path(SYSROOT),
         ...
         "-Wl,-rpath-link," + container_path(SYSROOT / "opt/aurorahyprland/lib"),
         "-Wl,-rpath-link," + container_path(SYSROOT / "usr/lib/aarch64-linux-gnu"),
         objects["CMakeFiles/Hyprland.dir/src/aurorahyprland/AndroidWlegl.cpp.o"], protocol_obj,
