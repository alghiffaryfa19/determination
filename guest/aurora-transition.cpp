/*
 * Aurora display-owner transition.
 *
...
void fail(const char *message) {
    std::fprintf(stderr, "aurora-transition: %s (egl=0x%x gl=0x%x)\n",
...
int main(int argc, char **argv) {
    ...
    if (argc == 2 && std::strcmp(argv[1], "phone") == 0) {
        ...
        std::fprintf(stderr, "usage: aurora-transition desktop|phone\n");
        return 2;
    }
    ...
    std::fprintf(stderr, "aurora-transition: %s complete (%dx%d, %dms)\n",
