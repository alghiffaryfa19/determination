...
#define AURORA_ANDROID_FORMAT_RGBA_8888 1u
#define AURORA_GRALLOC_USAGE_HW_TEXTURE UINT64_C(0x00000100)
#define AURORA_GRALLOC_USAGE_HW_RENDER UINT64_C(0x00000200)
#define AURORA_GRALLOC_USAGE_HW_COMPOSER UINT64_C(0x00000800)

typedef EGLBoolean(EGLAPIENTRYP aurora_create_native_buffer_fn)(
    EGLint, EGLint, EGLint, EGLint, EGLint *, EGLClientBuffer *);
typedef EGLBoolean(EGLAPIENTRYP aurora_release_native_buffer_fn)(EGLClientBuffer);
typedef void(EGLAPIENTRYP aurora_get_native_buffer_info_fn)(EGLClientBuffer,
                                                         int *, int *);
typedef void(EGLAPIENTRYP aurora_serialize_native_buffer_fn)(EGLClientBuffer,
                                                          int *, int *);

struct aurora_gralloc {
    EGLDisplay display;
    aurora_create_native_buffer_fn create;
    aurora_release_native_buffer_fn release;
    aurora_get_native_buffer_info_fn info;
    aurora_serialize_native_buffer_fn serialize;
};

struct aurora_pool_buffer {
    uint64_t id;
...
static void *required_proc(const char *name)
{
    ...
    if (!proc)
        fprintf(stderr, "aurora-gralloc-pool: missing %s\n", name);
    return proc;
}

static int gralloc_init(struct aurora_gralloc *gralloc)
{
    ...
    if (gralloc->display == EGL_NO_DISPLAY ||
        ...
        fprintf(stderr, "aurora-gralloc-pool: EGL init failed 0x%x\n",
                eglGetError());
        return -1;
    }
    gralloc->create = (aurora_create_native_buffer_fn)
        required_proc("eglHybrisCreateNativeBuffer");
    gralloc->release = (aurora_release_native_buffer_fn)
        required_proc("eglHybrisReleaseNativeBuffer");
    gralloc->info = (aurora_get_native_buffer_info_fn)
        required_proc("eglHybrisGetNativeBufferInfo");
    gralloc->serialize = (aurora_serialize_native_buffer_fn)
...
static int choose_payload_fd(const int *fds, int count)
{
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (fstat(fds[i], &info) == 0) {
            fprintf(stderr,
                    "aurora-gralloc-pool: handle fd[%d] size=%lld\n",
                                 ...
                                 uint32_t stride, uint64_t *modifier)
{
    ...
    *modifier = gbm_bo_get_modifier(bo);
    fprintf(stderr,
            "aurora-gralloc-pool: minigbm import PASS node=%s backend=%s "
...
static int allocate_buffer(struct aurora_gralloc *gralloc,
                           struct aurora_presenter_client *presenter,
                           struct aurora_pool_buffer *buffer, uint64_t id,
                           uint32_t width, uint32_t height,
                           const char *gbm_node)
{
    const uint64_t usage = AURORA_GRALLOC_USAGE_HW_TEXTURE |
                           AURORA_GRALLOC_USAGE_HW_RENDER |
                           AURORA_GRALLOC_USAGE_HW_COMPOSER;
    ...
    if (!gralloc->create((EGLint)width, (EGLint)height, (EGLint)usage,
                         AURORA_ANDROID_FORMAT_RGBA_8888, &buffer->stride,
    ...
    if (buffer->num_ints < 0 || buffer->num_ints > 128 ||
        buffer->num_fds <= 0 ||
        buffer->num_fds > (int)AURORA_GRALLOC_POOL_MAX_HANDLE_FDS) {
        errno = EOVERFLOW;
        return -1;
    }
    ...
    if (!buffer->minigbm_validated) {
        buffer->modifier = DRM_FORMAT_MOD_INVALID;
        fprintf(stderr,
                "aurora-gralloc-pool: minigbm rejected buffer %llu; "
                "advertising it for a direct Mesa dma-buf import attempt\n",
                (unsigned long long)id);
    }
    if (aurora_presenter_register_buffer(
            presenter, id, width, height, AURORA_ANDROID_FORMAT_RGBA_8888,
            (uint32_t)buffer->stride, usage, buffer->num_ints, buffer->ints,
            buffer->num_fds, buffer->fds) != 0)
        return -1;

    fprintf(stderr,
            "aurora-gralloc-pool: buffer=%llu stride=%dpx pitch=%uB "
            "handle=%dfd+%dint "
...
static void release_buffer(struct aurora_gralloc *gralloc,
                           struct aurora_pool_buffer *buffer)
{
    free(buffer->fds);
    free(buffer->ints);
    buffer->fds = NULL;
...
static int accept_compositor(int server, uid_t allowed_uid)
{
    ...
    if (getsockopt(client, SOL_SOCKET, SO_PEERCRED, &credential, &size) != 0 ||
        ...
        fprintf(stderr,
                "aurora-gralloc-pool: rejected compositor peer uid=%u\n",
                size == sizeof(credential) ? credential.uid : UINT32_MAX);
        close(client);
        errno = EACCES;
        return -1;
    }
    fprintf(stderr, "aurora-gralloc-pool: compositor connected uid=%u pid=%d\n",
            credential.uid, credential.pid);
    return client;
}

static int send_buffer_packet(int fd, const struct aurora_pool_buffer *buffer,
                              uint32_t width, uint32_t height,
                              uint32_t pool_size)
{
    const uint64_t usage = AURORA_GRALLOC_USAGE_HW_TEXTURE |
    ...
    if (buffer->minigbm_validated)
        packet.flags |= AURORA_GRALLOC_POOL_MINIGBM_VALIDATED;
    packet.buffer_id = buffer->id;
    packet.width = width;
    packet.height = height;
    packet.android_format = AURORA_ANDROID_FORMAT_RGBA_8888;
    ...
    char control[CMSG_SPACE(sizeof(int) * AURORA_GRALLOC_POOL_MAX_HANDLE_FDS)] = {0};
...
static int receive_present(int fd, struct aurora_gralloc_pool_packet *packet,
                           int *acquire_fence)
{
    struct iovec iov = {.iov_base = packet, .iov_len = sizeof(*packet)};
    ...
    *acquire_fence = -1;
    ...
    if (received != (ssize_t)sizeof(*packet) ||
        ...
        packet->op != AURORA_GRALLOC_POOL_PRESENT ||
        packet->serial == 0 || packet->buffer_id == 0 ||
        fd_count > 1 ||
        (!!(packet->flags & AURORA_GRALLOC_POOL_HAS_ACQUIRE_FENCE) !=
         (fd_count == 1))) {
        if (*acquire_fence >= 0)
            close(*acquire_fence);
        *acquire_fence = -1;
...
static int send_completion(int fd,
                           const struct aurora_presenter_packet *completion,
                           int present_fence, int release_fence)
{
    struct aurora_gralloc_pool_packet packet =
        aurora_gralloc_pool_packet_init(AURORA_GRALLOC_POOL_COMPLETE);
    ...
    if (present_fence >= 0) {
        packet.flags |= AURORA_GRALLOC_POOL_HAS_PRESENT_FENCE;
        fences[fence_count++] = present_fence;
    }
    if (release_fence >= 0) {
        packet.flags |= AURORA_GRALLOC_POOL_HAS_RELEASE_FENCE;
...
int main(int argc, char **argv)
{
    ...
    if (!aurora_gralloc_pool_dimensions_valid(width, height) ||
        !aurora_gralloc_pool_size_valid(pool_size)) {
        fprintf(stderr, "aurora-gralloc-pool: invalid geometry or pool size\n");
        return 2;
    }

    signal(SIGINT, stop_running);
    signal(SIGTERM, stop_running);
    struct aurora_gralloc gralloc;
    struct aurora_presenter_client presenter = {.fd = -1};
    struct aurora_pool_buffer buffers[AURORA_GRALLOC_POOL_MAX_BUFFERS] = {0};
    int server = -1;
    int compositor = -1;
    int result = 1;

    if (gralloc_init(&gralloc) != 0 ||
        aurora_presenter_connect(&presenter, presenter_socket) != 0) {
        fprintf(stderr, "aurora-gralloc-pool: presenter setup failed: %s\n",
                strerror(errno));
        goto out;
    }
    for (uint32_t i = 0; i < pool_size; ++i) {
        if (allocate_buffer(&gralloc, &presenter, &buffers[i], i + 1,
            ...
            fprintf(stderr,
                    "aurora-gralloc-pool: buffer %u allocation/register failed: "
                    "%s\n", i + 1, strerror(errno));
            goto out;
        }
    }
    ...
    if (server < 0) {
        fprintf(stderr, "aurora-gralloc-pool: listen %s failed: %s\n",
                pool_socket, strerror(errno));
        goto out;
    }
    fprintf(stderr,
            "aurora-gralloc-pool: ready socket=%s geometry=%ux%u pool=%u "
    ...
    while (running) {
        struct aurora_gralloc_pool_packet request;
        ...
        if (!buffer_id_valid(request.buffer_id, pool_size) ||
            aurora_presenter_present(&presenter, request.serial,
                                  request.buffer_id,
        ...
        if (poll(&ready, 1, 5000) <= 0 ||
            aurora_presenter_receive_completion(&presenter, &completion,
                                              &present_fence,
                                              &release_fence) != 0 ||
...
out:
    ...
    if (server >= 0)
        close(server);
    unlink(pool_socket);
    aurora_presenter_disconnect(&presenter);
    for (uint32_t i = 0; i < pool_size; ++i)
