...
cat > "$WORK/archive/etc/determination-profile" <<'EOF'
ID=arch
NAME=Arch Linux ARM
LIBC=glibc
INIT=systemd
...
cat > "$WORK/guests/alpine/rootfs/etc/determination-profile" <<'EOF'
ID=alpine
NAME=Alpine Linux
LIBC=musl
INIT=openrc
...
if DET="$WORK" DET_LXC_BIN="$WORK/lxc/bin" sh "$WORK/bin/guest-distro" select alpine; then
...
# $DET/guest anchor for LXC. Installing Debian later must replace only that
...
printf 'lxc.rootfs.path = dir:/data/determination/active-guest\n' > "$FRESH/guest/config"
...
DET="$FRESH" DET_LXC_BIN="$FRESH/lxc/bin" \
